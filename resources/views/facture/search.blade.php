    ...
    <div class="container-xl container-lg shadow mt-5">
        <table class="table table-striped">
            <tr>
                ...
                <th class="col-2">
                    <select name="type_fact_sec" id="" class="form-control text-center">
                        <option value="">Choix du type</option>
                        @foreach($type_sec as $type_f)
                            <option value="{{$type_f->nom_type_sec}}">{{$type_f->type_facture_sec}}</option>
                        @endforeach
                    </select>
                </th>
                <th>
                ...
                <th class="text-center text-nowrap">
                    ...
                    <a href="generate/facture" class="btn btn-secondary">genérer</a>
                </th>
                {!! Form::close() !!}
            </tr>
            <tr>
            ...
            @if(count($facture)>0)
                @foreach($facture as $fac)
                    <tr>
                        ...
                        <td class="text-center text-nowrap">{{$fac->type_fact_sec}}</td>
